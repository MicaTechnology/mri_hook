...
RSpec.describe MriHook::RequestHandlers::CreditBalanceSubmitterHandler do
  ...
  let(:payment_params) do
    {
      ...
      description: "Credit balance from MICA",
      batch_description: "Credit balance",
      check_url: nil,
      ...
      "value" => [
        {
          ...
          "ExternalBatchID" => "B1",
          "Description" => "Credit balance from MICA",
          "BatchDescription" => "Credit balance",
          "CheckURL" => nil,
  ...
  describe "#execute" do
    context "with valid parameters (no charge_id)" do
      ...
      it "does not require charge_id" do
        params = payment_params.merge(charge_id: nil)
        expect { handler.execute(params) }.not_to raise_error
      end
    end
    ...
    context "with parameters exceeding maximum length" do
      it "raises an ArgumentError when check_number exceeds maximum length" do
        params = payment_params.merge(check_number: "1" * 16)
      ...
      it "raises an ArgumentError when external_transaction_number exceeds maximum length" do
        params = payment_params.merge(external_transaction_number: "2" * 11)
      ...
      it "raises an ArgumentError when external_batch_id exceeds maximum length" do
        params = payment_params.merge(external_batch_id: "3" * 31)
      ...
      it "raises an ArgumentError when description exceeds maximum length" do
        params = payment_params.merge(description: "4" * 31)
      ...
      it "raises an ArgumentError when batch_description exceeds maximum length" do
        params = payment_params.merge(batch_description: "5" * 31)
    ...
    context "when the API returns no value key" do
      ...
      it "returns a Hash without response data" do
        result = handler.execute(payment_params)
    ...
    context "when the API returns an error" do
      let(:error_response) do
        {
          "odata.metadata" => "https://mrix5api.saas.mrisoftware.com/...",
          "value" => [
            {
      ...
      before do
        allow(handler.api_client).to receive(:post).and_return(error_response)
      end

      it "returns an error hash" do
        result = handler.execute(payment_params)
        expect(result[:status]).to eq(400)
        expect(result[:error]).to eq("Invalid payment")
      end
    end
  end
end
